...
async fn test_scenario_01_chat_app_core() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("chat");
            ...
            let sql = format!(
                "INSERT INTO {}.conversations (id, title) VALUES (100, 'U2 Conv')",
                ns
            );
            ...
            let resp = u1_client
                .execute_query(
                    &format!("DELETE FROM {}.messages WHERE id = 2", ns),
                    None,
                    None,
                )
                .await?;
            ...
            assert!(post_flush_count >= 54, "Post-flush count should be at least 54, got {}", post_flush_count);
            ...
            let resp = u1_client
                .execute_query(
                    &format!("SELECT id FROM {}.messages WHERE id = 2", ns),
                    None,
                    None,
                )
                .await?;
...
async fn test_scenario_01_service_writes_as_user() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("chat_svc");
            ...
            let resp = u1_client
                .execute_query(
                    &format!("INSERT INTO {}.messages (id, role_id, content) VALUES (1, 'user', 'Hello')", ns),
                    None,
                    None,
                )
                .await?;
