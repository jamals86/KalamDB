...
async fn test_scenario_09_ddl_while_active() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("ddl_active");
            ...
            for i in 1..=10 {
                let resp = client
                    .execute_query(
                        &format!("INSERT INTO {}.data (id, name, value) VALUES ({}, 'item_{}', {})", ns, i, i, i * 10),
                        None,
                        None,
                    )
                    .await?;
            ...
            let alter_resp = server
                .execute_sql(&format!(
                    "ALTER TABLE {}.data ADD COLUMN description TEXT",
                    ns
                ))
                .await?;
            ...
            let resp = client
                .execute_query(
                    &format!("SELECT COUNT(*) as cnt FROM {}.data", ns),
                    None,
                    None,
                )
                .await?;
...
async fn test_scenario_09_drop_column() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("ddl_drop");
            ...
            let drop_resp = server
                .execute_sql(&format!(
                    "ALTER TABLE {}.data DROP COLUMN old_column",
                    ns
                ))
                .await?;
