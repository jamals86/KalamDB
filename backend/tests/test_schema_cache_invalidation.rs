...
async fn test_cache_invalidation_removes_entry() {
    let (_schema_store, schema_cache) = setup_schema_infrastructure().await;

    // Note: register_system_tables pre-warms cache with system table schemas
    let initial_cache_size = schema_cache.len();
    ...
    // Insert into cache
    schema_cache.insert(table_id.clone(), table_def);

    // Verify entry exists
    assert!(
        schema_cache.get(&table_id).is_some(),
    ...
    assert!(
        schema_cache.get(&table_id).is_none(),
...
async fn test_cache_invalidation_forces_cache_miss() {
    ...
    schema_cache.insert(table_id.clone(), table_def.clone());
    for _ in 0..10 {
        let _schema = schema_cache.get(&table_id);
    }

    // Get cache stats before invalidation
    let (hits_before, misses_before, _, _) = schema_cache.stats();
    ...
    let result = schema_cache.get(&table_id);
    assert!(
        result.is_none(),
    ...
    let (hits_after, misses_after, _, _) = schema_cache.stats();
...
async fn test_selective_invalidation_preserves_other_entries() {
    ...
    schema_cache.invalidate(&table_2_id);
    ...
    assert!(
        schema_cache.get(&table_1_id).is_some(),
        "Table 1 should still be cached"
    );
    assert!(
        schema_cache.get(&table_2_id).is_none(),
        "Table 2 should be invalidated"
    );
    assert!(
        schema_cache.get(&table_3_id).is_some(),
...
async fn test_invalidation_idempotent() {
    ...
    schema_cache.invalidate(&table_id);
    schema_cache.invalidate(&table_id); // Second invalidation on non-existent entry
    schema_cache.invalidate(&table_id); // Third invalidation
...
async fn test_cache_stats_track_invalidation_behavior() {
    ...
    schema_cache.insert(table_id.clone(), table_def.clone());
    let _ = schema_cache.get(&table_id); // Hit
    schema_cache.invalidate(&table_id);
    let _ = schema_cache.get(&table_id); // Miss
    schema_cache.insert(table_id.clone(), table_def);
    let _ = schema_cache.get(&table_id); // Hit

    let (hits, misses, _, size) = schema_cache.stats();
