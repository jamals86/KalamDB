...
async fn test_create_table_path_resolution() {
    ...
    let create_sql = format!(
        "CREATE TABLE {}.{} (id INT PRIMARY KEY, name TEXT) WITH (TYPE = 'USER', FLUSH_POLICY = 'rows:10')",
        namespace, table
    );
    ...
    let path_result =
        table_cache.get_storage_path(&NamespaceId::new(namespace), &TableName::new(table));

    // Verify path was resolved
    assert!(
        path_result.is_ok(),
        "Storage path resolution should succeed"
    );
    ...
    assert!(
        partial_template.contains(namespace),
        "Path should contain namespace"
    );
    assert!(
        partial_template.contains(table),
        "Path should contain table name"
    );
...
async fn test_cache_hit_performance() {
    ...
    let response = server
        .execute_sql(&format!(
            "CREATE TABLE {}.{} (id INT PRIMARY KEY, value TEXT) WITH (TYPE = 'USER', FLUSH_POLICY = 'rows:10')",
            namespace, table
        ))
        .await;
    assert_eq!(response.status, "success");
    ...
    let path1 = table_cache
        .get_storage_path(&ns_id, &table_id)
        .expect("first path resolution");
    let first_duration = start.elapsed();

    // Second call - cache hit (from memory)
    let start = std::time::Instant::now();
    let path2 = table_cache
        .get_storage_path(&ns_id, &table_id)
    ...
    assert!(
        second_duration < std::time::Duration::from_millis(1),
        "Cache hit should be < 1ms, got: {:?}",
        second_duration
    );
...
async fn test_cache_invalidation() {
    ...
    assert_eq!(
        ...
        "Path should re-resolve to same value"
    );
...
async fn test_cache_hit_rate_many_queries() {
    ...
    for i in 0..10 {
        ...
        let response = server
            .execute_sql(&format!(
                "CREATE TABLE {}.{} (id INT PRIMARY KEY) WITH (TYPE = 'USER', FLUSH_POLICY = 'rows:10')",
                namespace, table
            ))
            .await;
    ...
    assert!(
        ...
        "Average cache hit should be < 100μs, got: {:?}",
        avg_duration
    );
...
async fn test_shared_table_path_resolution() {
    ...
    let response = server
        .execute_sql(&format!(
            "CREATE TABLE {}.{} (id INT, value TEXT) WITH (TYPE = 'SHARED', FLUSH_POLICY = 'rows:10')",
            namespace, table
        ))
        .await;
    assert_eq!(response.status, "success");
    ...
    let shared_path = table_cache
        .get_storage_path(&NamespaceId::new(namespace), &TableName::new(table))
        .expect("shared table path resolution");

    // Shared table should NOT have per-user {userId} placeholder
    assert!(
        !shared_path.contains("{userId}"),
        "Shared table should not have per-user paths, got: {}",
        shared_path
    );

    // Path should contain namespace and table name
    assert!(
        shared_path.contains(namespace),
        "Path should contain namespace"
    );
    assert!(
        shared_path.contains(table),
        "Path should contain table name"
    );
