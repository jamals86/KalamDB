...
impl ParquetWriter {
    ...
    pub fn write_with_bloom_filter(
        ...
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(KalamDbError::Io)?;
        }

        let file = File::create(path).map_err(KalamDbError::Io)?;
    ...
    pub fn write(&self, schema: SchemaRef, batches: Vec<RecordBatch>) -> Result<(), KalamDbError> {
...
mod tests {
    ...
    fn test_bloom_filter_enabled_for_updated_column() {
        let temp_dir = env::temp_dir().join("kalamdb_bloom_filter_test");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
    ...
    fn test_no_bloom_filter_without_updated_column() {
        let temp_dir = env::temp_dir().join("kalamdb_no_bloom_filter_test");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
    ...
    fn test_statistics_enabled() {
        let temp_dir = env::temp_dir().join("kalamdb_statistics_test");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
    ...
    fn test_compression_is_snappy() {
        let temp_dir = env::temp_dir().join("kalamdb_compression_test");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
