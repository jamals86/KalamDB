...
async fn test_user_table_subscription_debug() {
    ...
    with_http_test_server_timeout(Duration::from_secs(60), |server| {
        Box::pin(async move {
            ...
            // Get user_id
            let resp = server.execute_sql("SELECT user_id, username FROM system.users WHERE username='subuser'").await?;
            println!("SELECT user_id: status={:?}", resp.status);
            let rows = resp.rows_as_maps();
            println!("Users found: {:?}", rows);
            ...
            for i in 1..=5 {
                let insert_sql = format!("INSERT INTO {}.messages (id, content) VALUES ({}, 'message {}')", ns, i, i);
                let resp = client.execute_query(&insert_sql, None, None).await?;
            ...
            while tokio::time::Instant::now() < deadline {
                match tokio::time::timeout(Duration::from_millis(500), subscription.next()).await {
                    Ok(Some(Ok(ChangeEvent::Ack { subscription_id, total_rows: rows, .. }))) => {
            ...
            while tokio::time::Instant::now() < initial_deadline {
                match tokio::time::timeout(Duration::from_millis(500), subscription.next()).await {
                    Ok(Some(Ok(ChangeEvent::InitialDataBatch { rows, batch_control, .. }))) => {
                        println!("InitialDataBatch received: {} rows, has_more={}", rows.len(), batch_control.has_more);
