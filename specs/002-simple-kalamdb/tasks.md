...
- [ ] T012 [P] [Setup] Create new directory structure: `backend/crates/kalamdb-core/src/permissions/` for permission enforcement
...
- [ ] T022 [Foundation] Implement Arrow schema serialization/deserialization in `backend/crates/kalamdb-core/src/schema/arrow_schema.rs` (include table_options in schema files)
- [ ] T023 [Foundation] Implement schema versioning logic in `backend/crates/kalamdb-core/src/schema/versioning.rs` (create schema_v{N}.json files with user options)
...
- [ ] T025 [Foundation] Implement schema directory structure creation in `backend/crates/kalamdb-core/src/schema/storage.rs` (/configs/{namespace}/schemas/{table_name}/)
...
- [ ] T027 [Foundation] Implement column family manager in `backend/crates/kalamdb-core/src/storage/column_family_manager.rs` (create/delete column families with naming convention)
- [ ] T027a [Foundation] Add column family naming utilities in column_family_manager.rs (user_table:{namespace}:{table}, shared_table:{namespace}:{table}, stream_table:{namespace}:{table}, system_table:{table})
...
- [ ] T028 [Foundation] Implement catalog store using RocksDB in `backend/crates/kalamdb-core/src/catalog/catalog_store.rs` (store system table data in dedicated column families: system_table:users, system_table:live_queries, system_table:storage_locations, system_table:jobs)
- [ ] T029 [Foundation] Add catalog key prefixes for different entity types in catalog_store.rs (user_id:, subscription_id:, location_name:, job_id: for system tables)
- [ ] T030 [Foundation] Implement system table CRUD operations in catalog_store.rs (insert, update, delete, query system table data)
- [ ] T031 [Foundation] Implement table metadata cache in `backend/crates/kalamdb-core/src/catalog/table_cache.rs` (in-memory cache for table metadata loaded from JSON, NOT from RocksDB)

### DataFusion Integration Foundation

- [ ] T032 [Foundation] Create DataFusion session factory in `backend/crates/kalamdb-core/src/sql/datafusion_session.rs` (create SessionContext with config)
- [ ] T033 [Foundation] Implement hybrid TableProvider in `backend/crates/kalamdb-core/src/tables/hybrid_table_provider.rs` (queries both RocksDB and Parquet)
- [ ] T034 [Foundation] Create RocksDB scan implementation for DataFusion in `backend/crates/kalamdb-core/src/tables/rocksdb_scan.rs` (convert RocksDB data to Arrow RecordBatch, support different key formats per table type: {user_id}:{row_id} for user tables, {row_id} for shared tables, {timestamp}:{row_id} for stream tables)
...
- [ ] T038 [P] [Foundation] Create storage location template engine in `backend/crates/kalamdb-core/src/storage/path_template.rs` (substitute ${user_id} in paths)
...
- [ ] T040 [Foundation] Create system.users table schema in `backend/crates/kalamdb-core/src/tables/system/users.rs` (user_id, username, permissions array)
- [ ] T041 [Foundation] Create system.live_queries table schema in `backend/crates/kalamdb-core/src/tables/system/live_queries.rs` (live_id [PK, format: connection_id-query_id], connection_id, query_id, user_id, query, created_at, updated_at, changes, node)
...
- [ ] T043 [Foundation] Create system.jobs table schema in `backend/crates/kalamdb-core/src/tables/system/jobs.rs` (job_id, job_type, table_name, status, start_time, end_time, parameters, result, trace, memory_used_mb, cpu_used_percent, node_id, error_message)
...
- [ ] T064 [US1] Create namespace service in `backend/crates/kalamdb-core/src/services/namespace_service.rs` (create, list, update, delete operations)
...
- [ ] T067 [US1] Implement namespace creation in namespace_service.rs (create namespace entry, create schema directory structure /configs/{namespace}/)
- [ ] T068 [US1] Register DDL executors in `backend/crates/kalamdb-core/src/sql/executor.rs` (CREATE/ALTER/DROP/SHOW NAMESPACE)
- [ ] T069 [US1] Add namespace context to DataFusion session in datafusion_session.rs (track current namespace for table operations)
...
**Goal**: Manage users and permissions through system.users table with row-level filters and regex patterns

**Independent Test**: Insert users into system.users, assign permissions, verify permission enforcement in queries

### Implementation for User Story 2

- [ ] T070 [P] [US2] Implement system.users table provider in `backend/crates/kalamdb-core/src/tables/system/users_provider.rs` (TableProvider backed by RocksDB column family system_table:users)
- [ ] T071 [P] [US2] Create permission filter parser in `backend/crates/kalamdb-core/src/permissions/filter_parser.rs` (parse column filter expressions like "namespace.table.field = CURRENT_USER()")
- [ ] T072 [P] [US2] Create permission regex matcher in `backend/crates/kalamdb-core/src/permissions/regex_matcher.rs` (match table patterns like "namespace.messages.*")
- [ ] T073 [US2] Implement CURRENT_USER() function in `backend/crates/kalamdb-core/src/sql/functions/current_user.rs` (resolve from session context)
- [ ] T074 [US2] Create permission enforcement service in `backend/crates/kalamdb-core/src/permissions/enforcement.rs` (check permissions before query execution)
- [ ] T075 [US2] Implement permission filter injection into queries in enforcement.rs (add WHERE clauses based on user permissions)
- [ ] T076 [US2] Add user authentication context to DataFusion session in datafusion_session.rs (track current user_id for CURRENT_USER())
- [ ] T077 [US2] Implement INSERT/UPDATE/DELETE operations on system.users via SQL in users_provider.rs
- [ ] T078 [US2] Add permission precedence logic in enforcement.rs (handle multiple permissions per user)
- [ ] T079 [US2] Protect system.users table access in enforcement.rs (only administrators can modify)

**Checkpoint**: User management and permissions functional - can add users, assign permissions, enforce row-level security
...
- [ ] T081 [US2a] Create in-memory WebSocket connection registry in `backend/crates/kalamdb-core/src/live_query/connection_registry.rs` (struct ConnectedWebSocket { actor: Addr<WebSocketSession>, live_ids: Vec<String> }, HashMap<connection_id, ConnectedWebSocket>, HashMap<live_id, connection_id>, store current node_id)
...
- [ ] T083 [US2a] Add subscription registration in live query manager (on WebSocket connect: generate connection_id; on subscribe: generate live_id = connection_id + "-" + query_id, register in system.live_queries with live_id/connection_id/query_id/user_id/query/created_at/updated_at/changes=0/node, store in both HashMaps, add to ConnectedWebSocket.live_ids)
- [ ] T084 [US2a] Implement multi-subscription support per WebSocket connection (each connection can have multiple live_ids with different query_ids, track in ConnectedWebSocket.live_ids vector)
- [ ] T085 [US2a] Add subscription cleanup in live query manager (on disconnect: lookup ConnectedWebSocket by connection_id, for each live_id delete from system.live_queries and live_to_connection HashMap, remove connection from connections HashMap)
...
- [ ] T087 [US2a] Add node-aware notification delivery (filter subscriptions by node field, only deliver to local actors where node==current_node_id, lookup connection_id from live_id, get ConnectedWebSocket, extract query_id from live_id, send message to actor with query_id)
- [ ] T088 [US2a] Apply permission filters to system.live_queries queries in live_queries_provider.rs (users see only their own subscriptions by default)
- [ ] T099 [US2a] Implement KILL LIVE QUERY command parser in `backend/crates/kalamdb-core/src/sql/ddl/kill_live_query.rs` (parse KILL LIVE QUERY live_id)
- [ ] T100 [US2a] Add kill live query execution in live query manager (lookup connection_id from live_id, get ConnectedWebSocket, send disconnect message to actor, cleanup subscription)
...
- [ ] T099 [P] [US2b] Implement system.storage_locations table provider in `backend/crates/kalamdb-core/src/tables/system/storage_locations_provider.rs` (TableProvider backed by RocksDB column family system_table:storage_locations)
- [ ] T100 [US2b] Create storage location service in `backend/crates/kalamdb-core/src/services/storage_location_service.rs` (add, update, delete, resolve location)
- [ ] T099 [US2b] Add location name uniqueness validation in storage_location_service.rs
...
- [ ] T099 [US2b] Add deletion prevention for referenced locations in storage_location_service.rs (check usage_count > 0, return error with dependent tables)
- [ ] T100 [US2b] Implement INSERT/UPDATE/DELETE operations on system.storage_locations via SQL
- [ ] T099 [US2b] Add location accessibility validation in storage_location_service.rs (test filesystem/S3 path before adding)
- [ ] T100 [US2b] Protect system.storage_locations table access (only administrators can modify)
...
- [ ] T099 [P] [US2c] Implement system.jobs table provider in `backend/crates/kalamdb-core/src/tables/system/jobs_provider.rs` (TableProvider backed by RocksDB column family system_table:jobs)
- [ ] T100 [US2c] Create job execution framework in `backend/crates/kalamdb-core/src/jobs/executor.rs` (execute jobs, track metrics, update status)
- [ ] T101 [US2c] Add job registration in job executor (create job record with status='running' when job starts)
- [ ] T102 [US2c] Add job completion recording in job executor (update status='completed'/'failed', record end_time, result, trace, metrics)
...
- [ ] T107 [US2c] Apply permission filters to system.jobs queries (users see only their own table jobs by default)
...
- [ ] T109 [P] [US3] Implement CREATE USER TABLE parser in `backend/crates/kalamdb-core/src/sql/ddl/create_user_table.rs` (parse schema, LOCATION clause, LOCATION REFERENCE, FLUSH POLICY, deleted_retention)
- [ ] T110 [US3] Create user table service in `backend/crates/kalamdb-core/src/services/user_table_service.rs` (create table metadata, register in catalog, create column family)
...
- [ ] T112a [US3] Prevent system column injection for stream tables (stream tables do NOT have \_updated or \_deleted columns - this validation added in stream table service)
- [ ] T113 [US3] Implement storage location resolution in user_table_service.rs (resolve LOCATION REFERENCE or validate LOCATION path template)
- [ ] T114 [US3] Create schema file for user table in user_table_service.rs (generate schema_v1.json, update manifest.json, create current.json symlink)
- [ ] T115 [US3] Create column family for user table in user_table_service.rs (use column_family_manager to create user_table:{namespace}:{table_name})
- [ ] T116 [US3] Implement user table INSERT handler in `backend/crates/kalamdb-core/src/tables/user_table_insert.rs` (write to RocksDB column family with key format {user_id}:{row_id}, set \_updated = NOW(), \_deleted = false)
- [ ] T117 [US3] Implement user table UPDATE handler in `backend/crates/kalamdb-core/src/tables/user_table_update.rs` (update in RocksDB column family, set \_updated = NOW())
- [ ] T118 [US3] Implement user table DELETE handler (soft delete) in `backend/crates/kalamdb-core/src/tables/user_table_delete.rs` (set \_deleted = true, \_updated = NOW())
- [ ] T119 [US3] Create user table provider for DataFusion in `backend/crates/kalamdb-core/src/tables/user_table_provider.rs` (register table, provide schema, handle queries)
- [ ] T120 [US3] Implement user ID path substitution in user_table_provider.rs (replace ${user_id} in storage paths)
- [ ] T121 [US3] Add data isolation enforcement in user_table_provider.rs (queries only access current user's data by filtering on user_id key prefix)
- [ ] T122 [US3] Implement flush trigger logic in `backend/crates/kalamdb-core/src/flush/trigger.rs` (monitor row count and time intervals per column family)
- [ ] T123 [US3] Create flush job for user tables in `backend/crates/kalamdb-core/src/flush/user_table_flush.rs` (iterate column family, group rows by user_id prefix, write separate Parquet file per user at ${user_id}/batch-*.parquet, delete flushed rows from RocksDB)
- [ ] T124 [US3] Add deleted_retention configuration to table metadata in user_table_service.rs
- [ ] T125 [US3] Register flush jobs in system.jobs table (status, metrics, result, trace)
...
- [ ] T126 [P] [US3a] Implement DROP TABLE parser in `backend/crates/kalamdb-core/src/sql/ddl/drop_table.rs` (parse DROP USER TABLE and DROP SHARED TABLE)
- [ ] T127 [US3a] Create table deletion service in `backend/crates/kalamdb-core/src/services/table_deletion_service.rs` (orchestrate cleanup)
- [ ] T128 [US3a] Add active subscription check in table_deletion_service.rs (query system.live_queries, prevent drop if active subscriptions exist)
- [ ] T129 [US3a] Implement RocksDB buffer cleanup in table_deletion_service.rs (delete entire column family for table: user_table:{namespace}:{table_name} or shared_table:{namespace}:{table_name})
- [ ] T130 [US3a] Implement Parquet file deletion in table_deletion_service.rs (delete all user-specific Parquet files from storage)
- [ ] T131 [US3a] Add metadata removal in table_deletion_service.rs (remove from manifest.json, delete schema directory)
...
- [ ] T134 [US3a] Register DROP TABLE operations as jobs in system.jobs (track cleanup progress)
...
- [ ] T135 [P] [US3b] Implement ALTER TABLE parser in `backend/crates/kalamdb-core/src/sql/ddl/alter_table.rs` (parse ADD COLUMN, DROP COLUMN, MODIFY COLUMN)
- [ ] T136 [US3b] Create schema evolution service in `backend/crates/kalamdb-core/src/services/schema_evolution_service.rs` (orchestrate schema changes)
- [ ] T137 [US3b] Add ALTER TABLE validation in schema_evolution_service.rs (check backwards compatibility, validate type changes)
- [ ] T138 [US3b] Add subscription column reference check in schema_evolution_service.rs (prevent dropping columns referenced in active subscriptions)
- [ ] T139 [US3b] Prevent altering system columns in schema_evolution_service.rs (reject changes to \_updated, \_deleted)
- [ ] T140 [US3b] Prevent altering stream tables in schema_evolution_service.rs (stream table schemas are immutable)
- [ ] T141 [US3b] Implement schema version increment in schema_evolution_service.rs (create schema_v{N+1}.json)
...
- [ ] T144 [US3b] Invalidate schema cache in schema_evolution_service.rs (force DataFusion to reload schema)
...
- [ ] T146 [US3b] Add DESCRIBE TABLE enhancement to show schema history in `backend/crates/kalamdb-core/src/sql/ddl/describe_table.rs`
...
- [ ] T147 [P] [US4a] Implement CREATE STREAM TABLE parser in `backend/crates/kalamdb-core/src/sql/ddl/create_stream_table.rs` (parse schema, retention, ephemeral, max_buffer)
- [ ] T148 [US4a] Create stream table service in `backend/crates/kalamdb-core/src/services/stream_table_service.rs` (create table metadata, register in catalog, create column family, NO system columns)
- [ ] T149 [US4a] Create stream table provider in `backend/crates/kalamdb-core/src/tables/stream_table_provider.rs` (memory/RocksDB-only, no Parquet)
- [ ] T150 [US4a] Create column family for stream table in stream_table_service.rs (use column_family_manager to create stream_table:{namespace}:{table_name})
...
- [ ] T153 [US4a] Implement TTL-based eviction in `backend/crates/kalamdb-core/src/tables/stream_table_eviction.rs` (background job removes entries with timestamp older than retention period)
...
- [ ] T156 [US4a] Prevent Parquet flush for stream tables in flush trigger logic (check table type, skip stream tables)
- [ ] T157 [US4a] Add stream table metadata to DESCRIBE TABLE output
- [ ] T158 [US4a] Implement DROP STREAM TABLE support in drop_table.rs (delete column family stream_table:{namespace}:{table_name})
...
**Independent Test**: Create shared table, insert data from different users, verify all users see same data (filtered by permissions)

### Implementation for User Story 5

- [ ] T159 [P] [US5] Implement CREATE SHARED TABLE parser in `backend/crates/kalamdb-core/src/sql/ddl/create_shared_table.rs` (parse schema, LOCATION, FLUSH POLICY, deleted_retention)
- [ ] T160 [US5] Create shared table service in `backend/crates/kalamdb-core/src/services/shared_table_service.rs` (create table metadata, register in catalog, create column family)
- [ ] T161 [US5] Add system column injection in shared_table_service.rs (\_updated TIMESTAMP, \_deleted BOOLEAN for shared tables)
- [ ] T162 [US5] Create column family for shared table in shared_table_service.rs (use column_family_manager to create shared_table:{namespace}:{table_name})
...
- [ ] T164 [US5] Create shared table provider for DataFusion in `backend/crates/kalamdb-core/src/tables/shared_table_provider.rs` (single storage location, no user_id templating)
- [ ] T165 [US5] Apply permission filters to shared table queries in shared_table_provider.rs (row-level security per user)
- [ ] T166 [US5] Create flush job for shared tables in `backend/crates/kalamdb-core/src/flush/shared_table_flush.rs` (read all rows from column family, write to SINGLE Parquet file at shared/{table_name}/batch-*.parquet, delete flushed rows from RocksDB)
- [ ] T167 [US5] Add shared table support to DROP TABLE command (delete column family shared_table:{namespace}:{table_name})

**Checkpoint**: Shared tables functional - can create global tables, enforce permissions, flush to storage
...
- [ ] T168 [P] [US6] Create change notification generator in `backend/crates/kalamdb-core/src/live_query/change_detector.rs` (detect INSERT/UPDATE/DELETE from RocksDB writes)
...
- [ ] T174 [US6] Add change notification on flush completion in flush jobs (notify subscribers after Parquet write)
...
- [ ] T176 [US6] Add subscription isolation per user_id in live query manager (users only subscribe to their own data)
...
- [ ] T178 [P] [US7] Implement BACKUP DATABASE parser in `backend/crates/kalamdb-core/src/sql/ddl/backup_namespace.rs`
- [ ] T179 [P] [US7] Implement RESTORE DATABASE parser in `backend/crates/kalamdb-core/src/sql/ddl/restore_namespace.rs`
- [ ] T180 [P] [US7] Implement SHOW BACKUP parser in `backend/crates/kalamdb-core/src/sql/ddl/show_backup.rs`
- [ ] T181 [US7] Create backup service in `backend/crates/kalamdb-core/src/services/backup_service.rs` (orchestrate backup operations)
- [ ] T182 [US7] Implement manifest.json backup in backup_service.rs (save schema versions and table metadata)
- [ ] T183 [US7] Implement Parquet file backup in backup_service.rs (copy all Parquet files to backup location)
- [ ] T184 [US7] Include soft-deleted rows in backup (\_deleted=true rows preserved for change history)
- [ ] T185 [US7] Exclude stream tables from backup in backup_service.rs (ephemeral data not persisted)
- [ ] T186 [US7] Create restore service in `backend/crates/kalamdb-core/src/services/restore_service.rs` (restore schemas, tables, data)
...
- [ ] T190 [P] [US8] Implement DESCRIBE TABLE parser (already started in T146, enhance here)
...
- [ ] T192 [US8] Add table type indicator in SHOW TABLES output (USER, SHARED, STREAM, SYSTEM)
- [ ] T193 [US8] Add storage location to DESCRIBE TABLE output
- [ ] T194 [US8] Add flush policy to DESCRIBE TABLE output
- [ ] T195 [US8] Add stream configuration (retention, ephemeral, max_buffer) to DESCRIBE TABLE for stream tables
- [ ] T196 [US8] Add system columns (\_updated, \_deleted) to DESCRIBE TABLE output
- [ ] T197 [US8] Add schema version and history to DESCRIBE TABLE output (current version, schema file paths)
- [ ] T198 [US8] Implement table statistics query in `backend/crates/kalamdb-core/src/sql/ddl/show_table_stats.rs` (row counts, storage size)
...
- [ ] T200 [P] [Polish] Create example configuration file `backend/config.example.toml` with all new settings documented
...
- [ ] T206 [P] [Polish] Implement schema cache in DataFusion session factory (avoid repeated manifest.json reads)
- [ ] T207 [P] [Polish] Add query result caching for system table queries (catalog, live_queries, jobs)
- [ ] T208 [P] [Polish] Optimize Parquet bloom filter generation for \_updated column
- [ ] T209 [P] [Polish] Add metrics collection (query latency, flush job duration, WebSocket message throughput, column family sizes)
...
- [ ] T213 [P] [Polish] Validate user permissions before WebSocket subscription registration
...
- **User Stories 1-8 (Phases 4-16)**: All depend on Foundational and US0 completion
  ...
  - US2 (Users/Permissions): Independent, can start after US0
  ...
  - US5 (Shared Tables): Depends on US1 (namespaces), US2 (permissions)
...
5. **Phase 5**: US2 - User Management & Permissions
...
Tasks T069-T078 (system.users and permissions)
...
- **Permission Enforcement**: All queries go through permission filter injection
...
**Total Tasks**: 226 tasks (updated to include RocksDB column family architecture and in-memory actor registry)
**P1 Critical Tasks**: ~200 tasks (Phases 1-10, 12-13, and essential Polish)
...
- Namespace management with persistent configuration (configs/namespaces.json)
- User management with permissions
- **System tables**:
  - system.users (user management with permissions)
...
- Storage location management with persistent configuration (configs/storage_locations.json)
