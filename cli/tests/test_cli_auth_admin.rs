...
async fn test_cli_flush_all_tables() {
    ...
    let _ = execute_sql_as_root(
        "CREATE TABLE flush_all_test.table1 (id INT PRIMARY KEY, data VARCHAR) WITH (TYPE='USER', FLUSH_POLICY='rows:10')",
    )
    .await;
    let _ = execute_sql_as_root(
        "CREATE TABLE flush_all_test.table2 (id INT PRIMARY KEY, value DOUBLE) WITH (TYPE='USER', FLUSH_POLICY='rows:10')",
    )
    .await;
    tokio::time::sleep(Duration::from_millis(100)).await;
