...
async fn test_scenario_11_multi_storage_basic() -> anyhow::Result<()> {
    ...
    // =========================================================
            ...
            let resp = client
                .execute_query(
                    &format!("SELECT COUNT(*) as cnt FROM {}.hot_data", ns),
                    None,
                    None,
                )
                .await?;
            ...
            let resp = client
                .execute_query(
                    &format!("SELECT COUNT(*) as cnt FROM {}.cold_data", ns),
                    None,
                    None,
                )
                .await?;
    ...
    let ns = unique_ns("storage_constraints");
            ...
            for i in 1..=50 {
                let resp = client
                    .execute_query(
                        &format!(
                            "INSERT INTO {}.constrained (id, data) VALUES ({}, 'data_{}')",
                            ns, i, i
                        ),
                        None,
                        None,
            ...
            let resp = client
                .execute_query(
                    &format!("SELECT COUNT(*) as cnt FROM {}.constrained", ns),
                    None,
                    None,
                )
                .await?;
...
async fn test_scenario_11_table_types_storage() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("types_storage");
            ...
            let resp = user1_client
                .execute_query(
                    &format!("SELECT COUNT(*) as cnt FROM {}.user_table", ns),
                    None,
                    None,
                )
                .await?;
