...
async fn test_insert_with_simple_parameters() {
    ...
    server.execute_sql("CREATE TABLE default.test_insert (id INT PRIMARY KEY, name TEXT, age INT) WITH (TYPE = 'USER')").await;
...
async fn test_insert_multiple_rows_with_parameters() {
    ...
    // Create test table
    server.execute_sql("CREATE NAMESPACE default").await;
    server.execute_sql("CREATE TABLE default.test_insert_multi (id INT PRIMARY KEY, name TEXT) WITH (TYPE = 'USER')").await;
...
async fn test_update_with_simple_parameters() {
    ...
    server
        .execute_sql(
            "CREATE TABLE default.test_update (id TEXT PRIMARY KEY, name TEXT, age INT) WITH (TYPE = 'USER')",
        )
        .await;
...
async fn test_delete_with_simple_parameters() {
    ...
    server
        .execute_sql(
            "CREATE TABLE default.test_delete (id TEXT PRIMARY KEY, name TEXT) WITH (TYPE = 'USER')",
        )
        .await;
...
async fn test_dml_e2e_insert_update_delete() {
    ...
    server
        .execute_sql(
            "CREATE TABLE default.test_e2e (id TEXT PRIMARY KEY, name TEXT, age INT) WITH (TYPE = 'USER')",
        )
        .await;
