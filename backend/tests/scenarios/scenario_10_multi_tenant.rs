...
async fn test_scenario_10_multi_tenant_isolation() -> anyhow::Result<()> {
    ...
    let global = unique_ns("global");
            ...
            for (flag, enabled) in [("dark_mode", true), ("beta_features", false), ("analytics", true)] {
            ...
            // =========================================================
            create_test_users(server, &[("tenant_a_user", &Role::User), ("tenant_b_user", &Role::User)]).await?;
            ...
            let resp = tenant_a_client
                .execute_query(
                    &format!("SELECT * FROM {}.orders", tenant_a),
                    None,
                    None,
                )
                .await?;
            ...
            for row in &resp.rows_as_maps() {
                let name = row.get("customer_name")
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
                assert!(
            ...
            let resp = tenant_b_client
                .execute_query(
                    &format!("SELECT * FROM {}.orders", tenant_b),
                    None,
                    None,
                )
                .await?;
...
async fn test_scenario_10_same_table_name_different_namespaces() -> anyhow::Result<()> {
    ...
    let ns2 = unique_ns("same_name_2");
            ...
            create_test_users(server, &[("same_name_user1", &Role::User), ("same_name_user2", &Role::User)]).await?;
            ...
            let resp = client1
                .execute_query(
                    &format!("SELECT name FROM {}.users WHERE id = 1", ns1),
                    None,
                    None,
                )
                .await?;
            ...
            let resp = client2
                .execute_query(
                    &format!("SELECT name FROM {}.users WHERE id = 1", ns2),
                    None,
                    None,
                )
                .await?;
