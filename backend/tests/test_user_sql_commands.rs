...
async fn setup_test_executor() -> (
    ...
    // Create services (Phase 5: zero-sized structs)
    ...
    (executor, temp_dir, kalam_sql, users_provider, session_context)
}
...
async fn test_create_user_with_password_success() {
    let (executor, _temp_dir, _kalam_sql, users_provider, session_ctx) = setup_test_executor().await;
    ...
    let result = executor.execute(&*session_ctx, sql, Some(&admin_id)).await;
...
async fn test_create_user_without_authorization_fails() {
    ...
    let result = executor.execute(&*session_ctx, sql, Some(&regular_user_id)).await;
...
async fn test_alter_user_set_password() {
    ...
    executor
        .execute(&*session_ctx, create_sql, Some(&admin_id))
        .await
    ...
    let result = executor.execute(&*session_ctx, alter_sql, Some(&admin_id)).await;
...
async fn test_drop_user_soft_delete() {
    ...
    let result = executor.execute(&*session_ctx, drop_sql, Some(&admin_id)).await;
...
async fn test_alter_user_without_authorization_fails() {
    ...
    let result = executor.execute(&*session_ctx, alter_sql, Some(&regular_user_id)).await;
...
async fn test_drop_user_without_authorization_fails() {
    ...
    let result = executor.execute(&*session_ctx, drop_sql, Some(&regular_user_id)).await;
...
async fn test_create_user_password_length_validation() {
    ...
    let result_short = executor.execute(&*session_ctx, sql_short, Some(&admin_id)).await;
    ...
    let result_valid = executor.execute(&*session_ctx, sql_valid, Some(&admin_id)).await;
...
async fn test_create_user_duplicate_error() {
    ...
    let result1 = executor.execute(&*session_ctx, create_sql, Some(&admin_id)).await;
    ...
    let result2 = executor.execute(&*session_ctx, create_sql, Some(&admin_id)).await;
...
async fn test_drop_user_if_exists() {
    ...
    let result2 = executor.execute(&*session_ctx, drop_sql2, Some(&admin_id)).await;
...
async fn test_restore_deleted_user() {
    ...
    executor
        .execute(&*session_ctx, drop_sql, Some(&admin_id))
        .await
    ...
    let result = executor.execute(&*session_ctx, restore_sql, Some(&admin_id)).await;
...
async fn test_select_users_excludes_deleted() {
    ...
    executor
        .execute(&*session_ctx, create_sql1, Some(&admin_id))
        .await
        .expect("CREATE active_user failed");
    executor
        .execute(&*session_ctx, create_sql2, Some(&admin_id))
        .await
    ...
    let result = executor.execute(&*session_ctx, select_sql, Some(&admin_id)).await;
