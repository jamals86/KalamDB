...
async fn test_alter_table_add_column_after_flush() {
    ...
    assert_eq!(
        ...
        "Expected to flush 3 rows"
    );
    
    // Step 4: Alter table to add new column
    let alter_sql = format!(
        r#"ALTER TABLE {}.messages ADD COLUMN priority INT"#,
        ns
    );
...
async fn test_multiple_alter_operations_with_flushes() {
    ...
    server
        .execute_sql_as_user(
            &format!(
                "INSERT INTO {}.events (id, event_type) VALUES ('e1', 'login')",
                ns
            ),
            "user1",
        )
    ...
    server
        .execute_sql_as_user(
            &format!("ALTER TABLE {}.events ADD COLUMN user_id TEXT", ns),
            "user1",
        )
        .await;
