...
async function runTests() {
  ...
  try {
    const { default: init } = await import(join(pkgPath, 'kalam_link.js'));
    
    // For Node.js, we need to pass the WASM file path explicitly
    const wasmPath = join(pkgPath, 'kalam_link_bg.wasm');
  ...
  try {
    const { KalamClient } = await import(join(pkgPath, 'kalam_link.js'));
