...
async fn test_shared_table_create_and_drop() {
    ...
    // Verify table exists
    assert!(server.table_exists("test_ns", "conversations").await, "Table should exist");
    ...
    assert!(!server.table_exists("test_ns", "conversations").await, "Table should be dropped");
}
...
async fn test_shared_table_multiple_inserts() {
    ...
    let response = server.execute_sql(r#"
...
async fn test_shared_table_select() {
    ...
    // Select data
    let response = server.execute_sql(
        "SELECT * FROM test_ns.conversations ORDER BY conversation_id"
    ).await;
...
async fn test_shared_table_delete() {
    ...
    let response = server.execute_sql(
        "DELETE FROM test_ns.conversations WHERE name = 'conv001'"
    ).await;
    ...
    let response = server.execute_sql(
        "SELECT name FROM test_ns.conversations ORDER BY name"
    ).await;
...
async fn test_shared_table_system_columns() {
    ...
    let response = server.execute_sql(
        "SELECT name, value, _updated, _deleted FROM test_ns.conversations"
    ).await;
    ...
    assert!(!response.results[0].columns.is_empty(), "Should have columns");
}
...
async fn test_shared_table_flush_policy_rows() {
    ...
    let response = server.execute_sql(create_sql).await;
    assert_eq!(response.status, "success", "Failed to create table with FLUSH ROWS: {:?}", response.error);
...
async fn test_shared_table_query_filtering() {
    ...
    // Query with filter
    let response = server.execute_sql(
        "SELECT * FROM test_ns.conversations WHERE name = 'active'"
    ).await;
    
    assert_eq!(response.status, "success", "Failed to query with filter: {:?}", response.error);
}
...
async fn test_shared_table_ordering() {
    ...
    // Query with ORDER BY
    let response = server.execute_sql(
        "SELECT name FROM test_ns.conversations ORDER BY name ASC"
    ).await;
    
    assert_eq!(response.status, "success", "Failed to query with ORDER BY: {:?}", response.error);
}
...
async fn test_shared_table_drop_with_data() {
    ...
    for i in 0..5 {
        server.execute_sql(
            &format!(r#"INSERT INTO test_ns.conversations (name, value) VALUES ('conv{}', 'Data {}')"#, i, i)
        ).await;
    }
    
    // Drop table with data
    let response = fixtures::drop_table(&server, "test_ns", "conversations").await;
    assert_eq!(response.status, "success", "Failed to drop table with data: {:?}", response.error);
...
async fn test_shared_table_multiple_tables_same_namespace() {
    ...
    server.execute_sql(
        r#"INSERT INTO test_ns.conversations (name, value) VALUES ('conv1', 'Test')"#
    ).await;
    server.execute_sql(
        r#"INSERT INTO test_ns.config (name, value) VALUES ('setting1', 'value1')"#
    ).await;
    
    // Query both tables
    let response1 = server.execute_sql("SELECT * FROM test_ns.conversations").await;
...
async fn test_shared_table_complete_lifecycle() {
    ...
    // 4. Query data
    let response = server.execute_sql("SELECT * FROM lifecycle_test.conversations").await;
    assert_eq!(response.status, "success");
    ...
    // 6. Delete data
    server.execute_sql(
        "DELETE FROM lifecycle_test.conversations WHERE name = 'item2'"
    ).await;
