...
async fn test_time_based_flush_trigger() {
    ...
    for i in 1..=3 {
        ...
        fixtures::execute_sql(&server, &insert_sql, "user1").await.unwrap();
    }
    ...
    let jobs_query = "SELECT job_id, job_type, table_name, status FROM system.jobs WHERE job_type = 'flush'";
    let jobs_result = fixtures::execute_sql(&server, jobs_query, "user1").await;
    
    if let Ok(response) = jobs_result {
        println!("Flush jobs: {}", serde_json::to_string_pretty(&response).unwrap());
...
async fn test_combined_flush_trigger() {
    ...
    let result = fixtures::execute_sql(&server, &create_sql, "user1").await;
    assert!(result.is_ok(), "Failed to create table with combined triggers");
...
async fn test_multi_user_flush_grouping() {
    ...
    for user in &users {
        for i in 1..=3 {
            ...
            fixtures::execute_sql(&server, &insert_sql, user).await.unwrap();
        }
    }
...
async fn test_job_cancellation() {
    ...
    let jobs_query = "SELECT job_id, job_type, status FROM system.jobs WHERE status = 'running' LIMIT 1";
    let jobs_result = fixtures::execute_sql(&server, jobs_query, "user1").await;
    
    if let Ok(response) = jobs_result {
        println!("Running jobs: {}", serde_json::to_string_pretty(&response).unwrap());
