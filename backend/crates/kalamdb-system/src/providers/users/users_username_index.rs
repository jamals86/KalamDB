...
pub trait UsernameIndexExt {
    ...
    fn index_user(&self, user: &User) -> Result<(), SystemError>;

    /// Remove a user from the index
    fn remove_user(&self, username: &str) -> Result<(), SystemError>;

    /// Lookup a user by username
    fn lookup(&self, username: &str) -> Result<Option<UserId>, SystemError>;
}
