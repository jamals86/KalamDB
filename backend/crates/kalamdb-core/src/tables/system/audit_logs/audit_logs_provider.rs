...
impl AuditLogsTableProvider {
    ...
    pub fn append(&self, entry: AuditLogEntry) -> Result<(), KalamDbError> {
    ...
    pub fn get_entry(&self, audit_id: &AuditLogId) -> Result<Option<AuditLogEntry>, KalamDbError> {
        Ok(self.store.get(audit_id)?)
    }

    /// Scan all audit log entries and return as RecordBatch
    pub fn scan_all_entries(&self) -> Result<RecordBatch, KalamDbError> {
