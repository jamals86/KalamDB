...
use common::{TestServer, fixtures};
...
async fn test_01_create_namespace() {
    ...
    assert!(server.namespace_exists("app").await, "Namespace should exist");
}
...
async fn test_02_create_user_table() {
    ...
    // Verify table exists
    assert!(server.table_exists("app", "messages").await, "Table should exist");
}
...
async fn test_03_insert_data() {
    ...
    for (i, response) in responses.iter().enumerate() {
        assert_eq!(
            ...
            "Insert {} failed: {:?}", i, response.error
        );
    }
...
async fn test_05_update_data() {
    ...
    assert_eq!(response.status, "success", "Update failed: {:?}", response.error);
    
    // Verify update
    let query_response = server.execute_sql(
        "SELECT content FROM app.messages WHERE id = 1"
    ).await;
...
async fn test_06_delete_data() {
    ...
    assert_eq!(response.status, "success", "Delete failed: {:?}", response.error);
}
...
async fn test_07_create_shared_table() {
    ...
    // Verify table exists
    assert!(server.table_exists("app", "config").await, "Shared table should exist");
}
...
async fn test_08_insert_into_shared_table() {
    ...
    let response = server.execute_sql(
        r#"INSERT INTO app.config (name, value) VALUES ('max_connections', '100')"#
    ).await;
    
    assert_eq!(response.status, "success", "Insert into shared table failed: {:?}", response.error);
    
    // Query back
    let query_response = server.execute_sql(
        "SELECT * FROM app.config WHERE name = 'max_connections'"
    ).await;
...
async fn test_09_create_stream_table() {
    ...
    assert_eq!(response.status, "success", "Failed to create stream table: {:?}", response.error);
    
    // Verify table exists
    assert!(server.table_exists("app", "events").await, "Stream table should exist");
}
...
async fn test_10_insert_into_stream_table() {
    ...
    assert_eq!(response.status, "success", "Insert into stream table failed: {:?}", response.error);
}
...
async fn test_11_list_namespaces() {
    ...
    assert_eq!(response.status, "success", "Failed to query namespaces: {:?}", response.error);
}
...
async fn test_12_list_tables() {
    ...
    // Query system.tables
    let response = server.execute_sql("SELECT * FROM system.tables WHERE namespace = 'app'").await;
    
    assert_eq!(response.status, "success", "Failed to query tables: {:?}", response.error);
}
...
async fn test_13_query_system_users() {
    ...
    assert_eq!(response.status, "success", "Failed to query users: {:?}", response.error);
}
...
async fn test_14_drop_table() {
    ...
    assert!(!server.table_exists("app", "messages").await, "Table should be dropped");
}
...
async fn test_15_drop_namespace() {
    ...
    assert!(!server.namespace_exists("app").await, "Namespace should be dropped");
}
...
async fn test_17_performance_write_latency() {
    ...
    for i in 0..iterations {
        let start = Instant::now();
        let response = fixtures::insert_message(
            &server,
            "perf",
            "user123",
            &format!("Message {}", i)
...
async fn test_20_complete_environment_setup() {
    ...
    assert!(result.is_ok(), "Failed to setup environment: {:?}", result.err());
