...
async fn test_scenario_05_dashboards_shared_reference() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("app");
            ...
            let resp = user1_client
                .execute_query(
                    &format!("SELECT COUNT(*) as cnt FROM {}.activity", ns),
                    None,
                    None,
                )
                .await?;
            ...
            let resp = client
                .execute_query(
                    &format!("SELECT COUNT(*) as cnt FROM {}.plans", ns),
                    None,
                    None,
                )
                .await?;
            ...
            let resp = server
                .execute_sql(&format!(
                    "ALTER TABLE {}.activity ADD COLUMN device_type TEXT",
                    ns
                ))
                .await?;
...
async fn test_scenario_05_schema_evolution() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("app_schema");
            ...
            let resp = client
                .execute_query(
                    &format!("SELECT * FROM {}.events ORDER BY id", ns),
                    None,
                    None,
                )
                .await?;
