...
fn init_app_context() -> std::sync::Arc<AppContext> {
...
async fn test_typed_handler_create_namespace() {
    ...
    let exec_ctx = ExecutionContext::new(UserId::from("admin"), Role::Dba);
...
async fn test_classifier_prioritizes_select() {
    ...
    let executor = SqlExecutor::new(app_ctx, false);
    let session = SessionContext::new();
    let exec_ctx = ExecutionContext::new(UserId::from("user"), Role::User);
