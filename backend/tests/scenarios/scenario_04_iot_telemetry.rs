...
async fn test_scenario_04_iot_telemetry_5k_rows() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("iot");
            ...
            let resp = client
                .execute_query(
                    &format!("SELECT COUNT(*) as cnt FROM {}.telemetry", ns),
                    None,
                    None,
                )
                .await?;
            ...
            assert_eq!(
                post_flush_count, total_count,
                "Count should be same after flush"
            );
            ...
            assert_eq!(
                post_flush_device, device_count,
...
async fn test_scenario_04_anomaly_subscription() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("iot_anomaly");
            ...
            let sql = format!(
                "SELECT * FROM {}.telemetry WHERE temp > 50 OR battery < 15 ORDER BY id",
                ns
            );
