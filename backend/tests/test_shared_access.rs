...
async fn init_server() -> TestServer {
    ...
    let system_user = auth_helper::create_test_user(&server, system_username, system_password, Role::System).await;
    
    // Create default namespace as system user (use the correct user_id)
    let create_ns_sql = "CREATE NAMESPACE default";
    let result = server.execute_sql_as_user(create_ns_sql, system_user.id.as_str()).await;
...
async fn test_public_table_read_only_for_users() {
    ...
    create_shared_table_cf(&server, "default", "messages")
    ...
    assert!(is_success(&result), "Failed to create public table: {:?}", result.error);
    ...
    assert!(is_success(&result), "Regular user should be able to read public table: {:?}", result.error);
...
async fn test_change_access_level_requires_privileges() {
    ...
    let table = server.kalam_sql.get_table("default.config").unwrap().unwrap();
...
async fn test_access_level_only_on_shared_tables() {
    ...
    "#;
    let result = server.execute_sql_as_user(create_user_table_sql, service_username).await;
    assert!(
        is_error(&result),
    ...
    "#;
    let result = server.execute_sql_as_user(create_stream_table_sql, service_username).await;
    assert!(
        is_error(&result),
...
async fn test_alter_access_level_only_on_shared_tables() {
    ...
    "#;
    let result = server.execute_sql_as_user(create_user_table_sql, service_username).await;
    assert!(is_success(&result), "Failed to create USER table: {:?}", result.error);
