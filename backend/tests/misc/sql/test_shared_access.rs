...
async fn test_public_table_read_only_for_users() {
    ...
    let service_id = server
        .create_user(service_username, service_password, Role::Service)
        .await;
    ...
    let result = server
        .execute_sql_as_user(&create_table_sql, service_id_str)
        .await;
    assert_eq!(
        result.status,
        ResponseStatus::Success,
    ...
    let regular_id = server
        .create_user(regular_username, regular_password, Role::User)
        .await;
    ...
    let result = server
        .execute_sql_as_user(&select_sql, regular_id_str)
        .await;
    assert_eq!(
        result.status,
        ResponseStatus::Success,
    ...
    let result = server
        .execute_sql_as_user(&insert_sql, regular_id_str)
        .await;
    assert_eq!(
        result.status,
    ...
    let result = server
        .execute_sql_as_user(&update_sql, regular_id_str)
        .await;
    assert_eq!(
        result.status,
        ResponseStatus::Error,
    ...
    let result = server
        .execute_sql_as_user(&delete_sql, regular_id_str)
        .await;
    assert_eq!(
        result.status,
        ResponseStatus::Error,
...
async fn test_private_table_service_dba_only() {
    ...
    let dba_id = server
        .create_user(dba_username, dba_password, Role::Dba)
        .await;
    ...
    let result = server
        .execute_sql_as_user(&select_sql, service_id_str)
        .await;
    assert_eq!(
        result.status,
        ResponseStatus::Success,
    ...
    let result = server
        .execute_sql_as_user(&insert_sql, service_id_str)
        .await;
    assert_eq!(
        result.status,
        ResponseStatus::Success,
...
async fn test_change_access_level_requires_privileges() {
    ...
    let result = server
        .execute_sql_as_user(&alter_sql_private, dba_id_str)
        .await;
    assert_eq!(
        result.status,
        ResponseStatus::Success,
...
async fn test_user_cannot_modify_public_table() {
    ...
    let result = server
        .execute_sql_as_user(&update_sql, service_id_str)
        .await;
    assert_eq!(
        result.status,
        ResponseStatus::Success,
...
async fn test_access_level_only_on_shared_tables() {
    ...
    let result = server
        .execute_sql_as_user(&create_user_table_sql, service_id_str)
        .await;
    assert_eq!(
        result.status,
        ResponseStatus::Error,
    ...
    let result = server
        .execute_sql_as_user(&create_stream_table_sql, service_id_str)
        .await;
    assert_eq!(
        result.status,
        ResponseStatus::Error,
