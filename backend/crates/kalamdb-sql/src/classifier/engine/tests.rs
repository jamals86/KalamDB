...
fn test_authorization_check() {
    ...
    let stmt = SqlStatement::classify_and_parse("CREATE NAMESPACE test", &default_ns, Role::System)
        .unwrap();
    assert!(matches!(stmt.kind, SqlStatementKind::CreateNamespace(_)));

    // Regular user - cannot create namespace
    let err = SqlStatement::classify_and_parse("CREATE NAMESPACE test", &default_ns, Role::User)
        .unwrap_err();
    match err {
    ...
    let stmt =
        SqlStatement::classify_and_parse("SELECT * FROM users", &default_ns, Role::User).unwrap();
    assert!(matches!(stmt.kind, SqlStatementKind::Select));

    // Regular user - can show tables
    let stmt = SqlStatement::classify_and_parse("SHOW TABLES", &default_ns, Role::User).unwrap();
    assert!(matches!(stmt.kind, SqlStatementKind::ShowTables(_)));
}
