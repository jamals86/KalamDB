...
fn test_concurrent_users_isolation() {
    ...
    println!("\n🚀 Starting Concurrent Users Test ({} users, {} rows each)", NUM_USERS, ROWS_PER_USER);    
    ...
    if let Err(e) = execute_sql_as_root_via_cli(&format!("CREATE NAMESPACE IF NOT EXISTS {}", namespace)) {
    ...
    println!("✅ {} total rows inserted ({} per user) in {:.2?}", NUM_USERS * ROWS_PER_USER, ROWS_PER_USER, insert_elapsed);
    println!("   ├─ Thread times: min={:.2?}, avg={:.2?}, max={:.2?}", min_thread_time, avg_thread_time, max_thread_time);
    println!("   ├─ Server time (avg): {:.1}ms", avg_server_time);
    println!("   ├─ CLI total time (avg): {:.1}ms", avg_cli_time);
    println!("   ├─ CLI overhead (avg): {:.1}ms ({:.1}%)", avg_overhead, (avg_overhead / avg_cli_time) * 100.0);
    println!("   └─ Throughput: {:.1} inserts/sec", (NUM_USERS * ROWS_PER_USER) as f64 / insert_elapsed.as_secs_f64());
    ...
    for (username, password) in user_credentials.iter() {
        let timing = match execute_sql_via_cli_as_with_timing(username, password, &format!("SELECT * FROM {}", full_table)) {
            Ok(t) => t,
            Err(e) => {
        ...
        let row_count = timing.output.lines().filter(|l| {
            l.contains("│") && l.contains(|c: char| c.is_digit(10)) && !l.contains("─")
        }).count();
    ...
    let avg_verify_cli = verify_cli_times.iter().sum::<u128>() as f64 / verify_cli_times.len() as f64;
    ...
    println!("   └─ CLI overhead (avg): {:.1}ms ({:.1}%)", avg_verify_overhead, (avg_verify_overhead / avg_verify_cli) * 100.0);
    ...
    println!("   Breakdown:");
    println!("     ├─ Setup: {:.2?} ({:.1}%)", setup_start.elapsed(), (setup_start.elapsed().as_secs_f64() / total_time.as_secs_f64()) * 100.0);
    println!("     ├─ User creation: {:.2?} ({:.1}%)", user_creation_start.elapsed(), (user_creation_start.elapsed().as_secs_f64() / total_time.as_secs_f64()) * 100.0);
    println!("     ├─ Inserts: {:.2?} ({:.1}%)", insert_elapsed, (insert_elapsed.as_secs_f64() / total_time.as_secs_f64()) * 100.0);
    println!("     └─ Verification: {:.2?} ({:.1}%)", verify_start.elapsed(), (verify_start.elapsed().as_secs_f64() / total_time.as_secs_f64()) * 100.0);
}
