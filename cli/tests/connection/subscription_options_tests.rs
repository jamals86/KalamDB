...
fn test_subscription_options_has_resume_seq_id() {
    ...
    let opts_with = SubscriptionOptions::new()
        .with_from_seq_id(SeqId::from(123i64));
...
fn test_subscription_options_reconnection_workflow() {
    ...
    let initial_opts = SubscriptionOptions::new()
        .with_batch_size(100);
    
    assert!(!initial_opts.has_resume_seq_id(), "Initial subscription should not have resume seq_id");
    ...
    assert!(reconnect_opts.has_resume_seq_id(), "Reconnect subscription should have resume seq_id");
