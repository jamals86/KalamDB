...
async fn test_as_user_blocked_for_regular_user() {
    ...
    let create_table = format!(
        "CREATE USER TABLE {}.items (item_id VARCHAR PRIMARY KEY, name VARCHAR) STORAGE local",
        ns
    );
    ...
    let resp = server
        .execute_sql_as_user(&insert_sql, regular_user.as_str())
        .await;
...
async fn test_as_user_with_service_role() {
    ...
    server
        ...
        .await;
    let create_table = format!(
        "CREATE USER TABLE {}.orders (order_id VARCHAR PRIMARY KEY, amount VARCHAR) STORAGE local",
        ns
    );
    server
    ...
    let resp = server
        .execute_sql_as_user(&insert_sql, service_user.as_str())
        .await;
...
async fn test_as_user_with_dba_role() {
    ...
    server
        ...
        .await;
    let create_table = format!(
        "CREATE USER TABLE {}.logs (log_id VARCHAR PRIMARY KEY, message VARCHAR) STORAGE local",
        ns
    );
    server
    ...
    let resp = server
        .execute_sql_as_user(&insert_sql, dba_user.as_str())
        .await;
...
async fn test_insert_as_user_ownership() {
    ...
    server
        ...
        .await;
    let create_table = format!(
        "CREATE USER TABLE {}.messages (msg_id VARCHAR PRIMARY KEY, content VARCHAR) STORAGE local",
        ns
    );
    server
        .execute_sql_as_user(&create_table, admin_user.as_str())
...
async fn test_as_user_on_shared_table_rejected() {
    ...
    server
        ...
        .await;
    let create_table = format!(
        "CREATE SHARED TABLE {}.global_config (key VARCHAR PRIMARY KEY, value VARCHAR)",
        ns
    );
    server
        .execute_sql_as_user(&create_table, admin_user.as_str())
...
async fn test_as_user_performance() {
    ...
    server
        ...
        .await;
    let create_table = format!(
        "CREATE USER TABLE {}.perf_test (id VARCHAR PRIMARY KEY, data VARCHAR) STORAGE local",
        ns
    );
    server
        .execute_sql_as_user(&create_table, admin_user.as_str())
    ...
    println!(
        "Average AS USER operation time: {}ms (includes full INSERT execution)",
        avg_ms
    );
    ...
    assert!(
        avg_ms < 100,
        "AS USER operations took too long: {}ms average",
        avg_ms
    );
}
