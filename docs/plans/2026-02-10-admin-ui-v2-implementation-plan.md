...
REQUIRED UI SUB-SKILLS: Use `vercel-react-best-practices` and `vercel-composition-patterns` on every UI task.
...
- **React best practices are mandatory**:
  - Keep components pure and predictable.
  - Prefer composition over prop-flag explosion.
  - Keep data fetching in RTK Query/hooks, not leaf UI components.
  - Keep presentational components stateless where possible.
  - Use type-safe modeling with TypeScript and avoid `any`.
- **File-size limits (strict)**:
  - `*.tsx` component files: target <= 200 lines, hard cap 250.
  - Slice/API files: target <= 220 lines, hard cap 280.
  - If a file crosses target size, split immediately by concern.
- **Folder structure rule**:
  - Every feature uses subfolders: `components/`, `state/`, `api/`, `hooks/`, `selectors/`, `types.ts`.
  - Do not create monolithic files like `SqlStudio.tsx` with all logic.
- **Database/data access rule (strict)**:
  - Never place SQL statements or raw fetch logic inside pages/components.
  - All data access must go through feature services in `src/services/` (or `features/*/api` adapters calling shared services).
  - Define typed service methods (example: `notificationsService.subscribeUserNotifications`, `sqlService.execute`, `schemaService.describeTable`).
  - Components may dispatch actions/call hooks only; they must not construct SQL strings directly.
  - Shared query templates/constants must live in dedicated files under `src/services/sql/queries/`.
- **SQL Studio decomposition rule**:
  - Tab state, live state, pending-changes state, and explorer state must be separate modules.
  - Editor, explorer tree, favorites, results grid, and DDL drawer must be separate components.
- **Review gate before commit**:
  - Check for long-file drift and split before each commit.
