...
#[test]
fn shared_table_delete_1() -> anyhow::Result<()> {
    // Setup and insert data
    setup_benchmark_tables()?;
    std::thread::sleep(Duration::from_millis(200));

    let sql = "INSERT INTO bench_shared.items (id, value) VALUES (1, 'benchmark_value_1')";
    ...
    // Execute delete
    let delete_sql = "DELETE FROM bench_shared.items WHERE id = 1";
    let execution = execute_cli_timed_root(delete_sql)?;
