...
impl FilterPredicate {
    ...
    pub fn new(where_clause: &str) -> Result<Self, KalamDbError> {
        ...
        let statements = Parser::parse_sql(&dialect, &sql)
            .map_err(|e| KalamDbError::InvalidOperation(format!("Failed to parse WHERE clause: {}", e)))?;
        ...
        let expr = match &statements[0] {
            Statement::Query(query) => {
                if let Some(selection) = &query.body.as_select().and_then(|s| s.selection.as_ref()) {
                    ...
                    return Err(KalamDbError::InvalidOperation("No WHERE clause found".to_string()));
                }
            }
            _ => {
                return Err(KalamDbError::InvalidOperation("Invalid WHERE clause syntax".to_string()));
            }
        };
    ...
    fn compare_numeric(left: &JsonValue, right: &JsonValue, op: &str) -> Result<bool, KalamDbError> {
        ...
        let left_num = match left {
            ...
            _ => return Err(KalamDbError::InvalidOperation(format!(
...
mod tests {
    ...
    fn test_complex_filter() {
        let filter = FilterPredicate::new(
            "(user_id = 'user1' OR user_id = 'user2') AND read = false"
        ).unwrap();
    ...
    fn test_filter_cache() {
        ...
        cache.insert("live1".to_string(), "user_id = 'user1'").unwrap();
