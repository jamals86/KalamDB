...
fn create_temp_store() -> (FileCredentialStore, TempDir) {
    ...
    let store = FileCredentialStore::with_path(creds_path)
...
fn test_cli_multiple_instances() {
    ...
    let instance_list = store.list_instances()
    ...
    for (instance, username, password) in &instances {
        let retrieved = store.get_credentials(instance)
...
fn test_cli_credential_rotation() {
    ...
    store.set_credentials(&creds_v1)
        .expect("Failed to store initial credentials");
    
    // Retrieve initial credentials
    let retrieved_v1 = store.get_credentials("production")
        .expect("Failed to get credentials")
        .expect("Credentials should exist");
    ...
    store.set_credentials(&creds_v2)
        .expect("Failed to update credentials");
    
    // Retrieve updated credentials
    let retrieved_v2 = store.get_credentials("production")
        .expect("Failed to get credentials")
        .expect("Credentials should exist");
...
fn test_cli_delete_credentials() {
    ...
    // Verify it exists
    assert!(store.get_credentials("temp_instance")
        .expect("Failed to get credentials")
        .is_some());
    
    // Delete credentials
    store.delete_credentials("temp_instance")
...
fn test_cli_credentials_with_server_url() {
    ...
    // Retrieve and verify
    let retrieved = store.get_credentials("cloud")
        .expect("Failed to get credentials")
        .expect("Credentials should exist");
...
fn test_cli_empty_store() {
    ...
    let creds = store.get_credentials("nonexistent")
        .expect("Failed to get credentials");
