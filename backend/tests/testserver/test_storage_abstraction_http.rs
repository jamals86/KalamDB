...
fn file_uri(path: PathBuf) -> String {
    // `file://` + absolute POSIX path yields `file:///...`
...
async fn test_storage_abstraction_over_http() {
    test_support::http_server::with_http_test_server_timeout(Duration::from_secs(45), |server| {
        ...
        let response = server
            .execute_sql("CREATE NAMESPACE IF NOT EXISTS test_rocksdb_trait")
            .await?;
        assert_eq!(
            response.status,
            ResponseStatus::Success,
        ...
        let response = server
            .execute_sql("SELECT * FROM test_rocksdb_trait.test_table")
            .await?;
        assert_eq!(
            response.status,
            ResponseStatus::Success,
        ...
        let response = server
            .execute_sql("CREATE NAMESPACE IF NOT EXISTS test_partition_ns")
            .await?;
        assert_eq!(
            response.status,
            ResponseStatus::Success,
        ...
        assert_eq!(response.status, ResponseStatus::Error);
        let response = server
            .execute_sql("INSERT INTO nonexistent.table (id) VALUES (1)")
            .await?;
        assert_eq!(response.status, ResponseStatus::Error);
