...
use kalamdb_store::{UserTableStore, test_utils::TestDb};
use std::sync::Arc;
use serde_json::json;
...
fn bench_rocksdb_writes(c: &mut Criterion) {
    ...
    // Setup test database
    let test_db = TestDb::single_cf("user_table:bench:messages")
        .expect("Failed to create test database");
    
    let store = UserTableStore::new(test_db.db.clone())
    ...
    group.bench_function("single_write", |b| {
        ...
        b.iter(|| {
            ...
            store.put("bench", "messages", user_id, &row_id, row_data)
...
fn bench_rocksdb_reads(c: &mut Criterion) {
    ...
    for i in 0..1000u64 {
        ...
        store.put("bench", "messages", user_id, &row_id, row_data).expect("Failed to write");
    }

    // Benchmark single read
    group.bench_function("single_read", |b| {
        b.iter(|| {
            let result = store.get("bench", "messages", user_id, "500").expect("Failed to read");
            black_box(result);
        });
    });
    ...
    group.bench_function("scan_user", |b| {
        b.iter(|| {
            let result = store.scan_user("bench", "messages", user_id).expect("Failed to scan");
