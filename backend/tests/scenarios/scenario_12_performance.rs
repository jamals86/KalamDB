...
async fn test_scenario_12_insert_performance() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("perf_insert");
            ...
            let resp = client
                .execute_query(
                    &format!("SELECT COUNT(*) as cnt FROM {}.metrics", ns),
                    None,
                    None,
                )
                .await?;
...
async fn test_scenario_12_query_time_growth() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("perf_query");
            ...
            for &batch_target in &batch_sizes {
                    ...
                    let resp = client
                        .execute_query(
                            &format!(
                                ...
                                ns, id_counter, id_counter % 10, id_counter as f64 * 0.5
                            ),
                            None,
                            None,
                        )
                ...
                let resp = client
                    .execute_query(
                        &format!("SELECT * FROM {}.readings ORDER BY id", ns),
                        None,
                        None,
                    )
                    .await?;
            ...
            let time_growth = if first_time > 0.0 { last_time / first_time } else { 1.0 };
            assert!(
...
async fn test_scenario_12_subscription_snapshot_timing() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("perf_sub");
            ...
            for &data_size in &data_sizes {
                ...
                let _ = client
                    .execute_query(
                        &format!("DELETE FROM {}.documents WHERE id > 0", ns),
                        None,
                        None,
                    )
                    .await;
                ...
                let mut sub = client
                    .subscribe(&format!("SELECT * FROM {}.documents", ns))
                    .await?;
...
async fn test_scenario_12_memory_baseline() -> anyhow::Result<()> {
    ...
    let ns = unique_ns("perf_memory");
            ...
            let resp = client
                .execute_query(
                    &format!("SELECT COUNT(*) as cnt FROM {}.large_data", ns),
                    None,
                    None,
                )
                .await?;
