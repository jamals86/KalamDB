...
#[test]
fn stream_table_select_50k() -> anyhow::Result<()> {
    ...
    for batch in 0..num_batches {
        ...
        for i in 0..batch_size {
            ...
            values.push(format!("('type_{}', 'payload_{}')", idx, idx));
        }
        
        let sql = format!(
            "INSERT INTO bench_stream.events (event_type, payload) VALUES {}",
            values.join(", ")
        );
    ...
    // Execute select
    let select_sql = "SELECT * FROM bench_stream.events LIMIT 50000";
    ...
    let mut result = TestResult::new(
        ...
        "select",
        "Select 50,000 rows from stream table",
    );
