...
async fn test_column_id_stability_across_schema_changes() {
    ...
    println!("✅ Initial column_ids: id={}, name={}, price={}", id_col_id, name_col_id, price_col_id);
    ...
    assert_eq!(id_col_v2["column_id"].as_u64().unwrap(), id_col_id, "id column_id should not change");
    assert_eq!(name_col_v2["column_id"].as_u64().unwrap(), name_col_id, "name column_id should not change");
    assert_eq!(price_col_v2["column_id"].as_u64().unwrap(), price_col_id, "price column_id should not change");
    ...
    assert!(schema_v3.iter().all(|c| c["column_name"].as_str().unwrap() != "stock"), 
    ...
    assert!(category_col_id > stock_col_id, "New column_id should be greater than dropped column's ID");
    ...
    assert_eq!(resp.status, ResponseStatus::Success, "Failed to rename column: {:?}", resp.error);
    ...
    println!("✅ After RENAME: product_name column_id={} (was name with id={})", renamed_col_id, name_col_id);
    assert_eq!(renamed_col_id, name_col_id, "RENAME should preserve column_id");
    
    // Verify old name is gone
    assert!(schema_v5.iter().all(|c| c["column_name"].as_str().unwrap() != "name"), 
...
async fn test_column_stats_use_column_id() {
    ...
    assert_eq!(resp.status, ResponseStatus::Success, "Query after rename failed: {:?}", resp.error);
    ...
    assert_eq!(resp.status, ResponseStatus::Success, "Failed to insert with renamed column: {:?}", resp.error);
...
async fn test_full_lifecycle_with_alter_and_flush() {
    ...
    let resp = server.execute_sql_as_user(&insert_sql, "user1").await;
    assert_eq!(resp.status, ResponseStatus::Success, "Failed to insert initial data: {:?}", resp.error);
    ...
    assert_eq!(resp.status, ResponseStatus::Success, "Failed to add age column: {:?}", resp.error);
    ...
    assert_eq!(resp.status, ResponseStatus::Success, "Query after first ALTER failed: {:?}", resp.error);
    ...
    let resp = server.execute_sql_as_user(&alter_add2, "user1").await;
    assert_eq!(resp.status, ResponseStatus::Success, "Failed to add status column: {:?}", resp.error);
    println!("✅ Added column: status");
    
    // ========== Step 7: Drop a previous column ==========
    let alter_drop = format!(r#"ALTER TABLE {}.users DROP COLUMN email"#, ns);
    let resp = server.execute_sql_as_user(&alter_drop, "user1").await;
    assert_eq!(resp.status, ResponseStatus::Success, "Failed to drop email column: {:?}", resp.error);
    ...
    assert_eq!(column_names, vec!["id", "username", "age", "status"], 
    ...
    assert_eq!(resp.status, ResponseStatus::Success, "Failed to insert new data: {:?}", resp.error);
    ...
    assert!(!result_columns.contains(&"email".to_string()), 
...
async fn get_table_schema(server: &TestServer, namespace: &str, table_name: &str) -> Vec<HashMap<String, Value>> {
    ...
    let columns_str = rows[0]["columns"].as_str()
        .expect("columns field should be a string");
    
    // Parse the JSON string into a Vec of column definitions
    let all_columns: Vec<HashMap<String, Value>> = serde_json::from_str(columns_str)
        .expect("Failed to parse columns JSON");
    
    // Filter out system columns (_seq, _deleted)
    all_columns.into_iter()
        .filter(|col| {
            let name = col.get("column_name")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            !name.starts_with('_')
        })
...
fn find_column<'a>(schema: &'a [HashMap<String, Value>], column_name: &str) -> &'a HashMap<String, Value> {
    schema
        .iter()
