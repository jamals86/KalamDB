...
- **Gemini-backed agent** - Uses Vercel AI SDK (`ai` + `@ai-sdk/google`)
...
# 4. Configure Gemini key in .env.local (generated by setup.sh)
# Set GEMINI_API_KEY=...

# 5. Start the AI processor service (in one terminal)
npm run service

# 6. Start the Next.js dev server (in another terminal)
npm run dev

# 7. Open browser to any design
...
│   │   ├── service-config.ts    # Env loading and validation
...
| `NEXT_PUBLIC_KALAMDB_USERNAME` | Frontend username (public env) | none |
| `NEXT_PUBLIC_KALAMDB_PASSWORD` | Frontend password (public env) | none |
| `KALAMDB_URL` | KalamDB server URL (service) | `http://localhost:8080` |
| `KALAMDB_USERNAME` | Service user username | none |
| `KALAMDB_PASSWORD` | Service user password | none |
| `GEMINI_API_KEY` | Gemini API key used by service | none |
| `GEMINI_MODEL` | Gemini model id | `gemini-2.5-flash` |
| `AI_MAX_OUTPUT_TOKENS` | Max output tokens per response | `512` |
| `AI_TEMPERATURE` | Gemini temperature | `0.7` |
| `AI_CONTEXT_WINDOW_MESSAGES` | Number of recent messages sent as context | `20` |

Security note: `NEXT_PUBLIC_*` variables are exposed to the browser by Next.js and are not secrets.
