...
async fn test_system_cluster_views_over_http() -> Result<()> {
    ...
    let result = async {
        ...
        anyhow::ensure!(resp.status == ResponseStatus::Success, "system.cluster failed: {:?}", resp.error);
        ...
        anyhow::ensure!(first.contains_key("group_type"), "system.cluster_groups missing group_type");
