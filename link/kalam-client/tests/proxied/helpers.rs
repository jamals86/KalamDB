...
pub fn create_test_client() -> Result<KalamLinkClient, kalam_client::KalamLinkError> {
...
pub fn create_test_client_for_base_url(
    base_url: &str,
) -> Result<KalamLinkClient, kalam_client::KalamLinkError> {
...
pub fn create_test_client_with_events_for_base_url(
    base_url: &str,
) -> Result<(KalamLinkClient, Arc<AtomicU32>, Arc<AtomicU32>), kalam_client::KalamLinkError> {
    let token = common::isolated_root_access_token_blocking()
        .map_err(|e| kalam_client::KalamLinkError::ConfigurationError(e.to_string()))?;
...
pub async fn wait_for_disconnect_count(
    disconnect_count: &Arc<AtomicU32>,
    expected_disconnects: u32,
    context: &str,
) {
    let deadline = Instant::now() + RECONNECT_WAIT_TIMEOUT;

    loop {
        let current_disconnects = disconnect_count.load(Ordering::SeqCst);
        if current_disconnects >= expected_disconnects {
            return;
        }
...
pub async fn wait_until_connected(client: &KalamLinkClient, context: &str) {
    let deadline = Instant::now() + RECONNECT_WAIT_TIMEOUT;
