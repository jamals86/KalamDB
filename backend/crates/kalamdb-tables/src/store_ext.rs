...
use crate::{UserTableRow, UserTableStore, SharedTableRow, SharedTableStore, StreamTableRow, StreamTableStore};
use kalamdb_commons::ids::{UserTableRowId, SharedTableRowId, StreamTableRowId};
use kalamdb_store::entity_store::EntityStore;

/// Extension trait for user table stores
pub trait UserTableStoreExt {
    fn scan_user(&self, user_key_prefix: &UserTableRowId) -> Result<Vec<(UserTableRowId, UserTableRow)>, TableError>;
        ...
        raw_results
            ...
            .map(|(key_bytes, value)| {
                let key = UserTableRowId::from_bytes(&key_bytes)
                    .map_err(|e| TableError::Serialization(format!("Failed to deserialize key: {}", e)))?;
                Ok((key, value))
            })
            .collect()
    }
...
impl SharedTableStoreExt for SharedTableStore {
    fn scan_all_shared(&self) -> Result<Vec<(SharedTableRowId, SharedTableRow)>, TableError> {
        let raw_results = EntityStore::scan_all(self)
            .map_err(|e| TableError::Storage(e.to_string()))?;
