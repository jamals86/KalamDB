...
async fn test_as_user_blocked_for_regular_user() {
    ...
    assert_eq!(resp.status, "error", "Regular user should not be able to use AS USER");
    ...
    assert!(
        ...
        "Error should mention AS USER authorization: {}", error_msg
    );
}
...
async fn test_as_user_with_service_role() {
    ...
    assert_eq!(resp.status, "success", "Service role should be able to use AS USER");
    ...
    assert_eq!(resp.results[0].rows.as_ref().unwrap().len(), 1, "Target user should see their record");
}
...
async fn test_as_user_with_dba_role() {
    ...
    assert_eq!(resp.status, "success", "DBA role should be able to use AS USER");
    assert_eq!(resp.results[0].row_count, 1);
}
...
async fn test_insert_as_user_ownership() {
    ...
    assert_eq!(resp.status, "success");
    assert_eq!(resp.results[0].rows.as_ref().unwrap().len(), 0, "Bob should not see Alice's record");
}
...
async fn test_update_as_user() {
    ...
    let select_sql = format!("SELECT status FROM {}.profiles WHERE profile_id = 'PROF-1'", ns);
...
async fn test_as_user_on_shared_table_rejected() {
    ...
    server.execute_sql(&format!("CREATE NAMESPACE {}", ns)).await;
    let create_table = format!("CREATE SHARED TABLE {}.global_config (key VARCHAR PRIMARY KEY, value VARCHAR)", ns);
    server.execute_sql_as_user(&create_table, admin_user.as_str()).await;
    ...
    assert_eq!(resp.status, "error", "AS USER should be rejected on SHARED tables");
    let error_msg = resp.error.as_ref().unwrap().message.as_str();
    assert!(
        error_msg.contains("SHARED") && error_msg.contains("AS USER"),
        "Error should mention SHARED tables and AS USER: {}", error_msg
    );
}
