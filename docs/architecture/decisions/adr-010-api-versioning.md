...
1. **Healthcheck Response** (current):
```json
{
  "status": "healthy",
   "api_version": "v1"
...
2. **Documentation**: `docs/api/api-reference.md` documents all supported versions

3. **Error Response** for invalid version (planned):
```json
{
   "status": "error",
   "results": [],
   ...
   "error": {
      ...
      "message": "API version 'v3' not supported. Supported versions: v1, v2",
      "details": null
   }
}
```
