...
impl DeletedAtIndex {
    ...
    pub fn lookup_before(&self, before_date: &DateTime<Utc>) -> Result<Vec<UserId>, SystemError> {
    ...
    pub fn lookup_on_date(&self, date: &DateTime<Utc>) -> Result<Vec<UserId>, SystemError> {
    ...
    fn add_user_entry(&self, date: &DateTime<Utc>, user_id: &UserId) -> Result<(), SystemError> {
        let date_key = datetime_to_key(date);
        let user_id_str = user_id.as_str().to_string();
    ...
    fn get_entry(&self, key: &[u8]) -> Result<Option<DeletedAtIndexEntry>, SystemError> {
    ...
    fn put_entry(&self, key: &[u8], entry: &DeletedAtIndexEntry) -> Result<(), SystemError> {
    ...
    fn delete_entry(&self, key: &[u8]) -> Result<(), SystemError> {
