...
use kalam_link::{ConnectionOptions, SubscriptionOptions, SeqId};
...
fn test_multiple_subscriptions_seq_id_tracking() {
    ...
    subscriptions.insert("sub-1".to_string(), MockSubscriptionState {
        sql: "SELECT * FROM messages".to_string(),
        options: SubscriptionOptions::new().with_batch_size(50),
        last_seq_id: None,
    });
    
    subscriptions.insert("sub-2".to_string(), MockSubscriptionState {
...
fn test_infinite_reconnect_attempts() {
    let opts = ConnectionOptions::new()
        .with_max_reconnect_attempts(None); // Infinite
    ...
    for attempt in 0..100 {
        let should_retry = opts.max_reconnect_attempts
            .map(|max| attempt < max)
            .unwrap_or(true); // None means always retry
...
fn test_full_reconnection_workflow() {
    ...
    while is_reconnecting && reconnect_attempt < conn_opts.max_reconnect_attempts.unwrap_or(u32::MAX) {
        reconnect_attempt += 1;
