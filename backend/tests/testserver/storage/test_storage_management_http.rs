...
async fn test_storage_management_over_http() {
    with_http_test_server_timeout(Duration::from_secs(60), |server| {
        Box::pin(async move {
            ...
            anyhow::ensure!(resp.status == ResponseStatus::Success, "SHOW STORAGES failed: {:?}", resp.error);
            let rows = resp.results.first().map(|r| r.rows_as_maps()).unwrap_or_default();
            anyhow::ensure!(
                rows.iter().any(|r| r.get("storage_id").and_then(|v| v.as_str()) == Some("local")),
            ...
            anyhow::ensure!(resp.status == ResponseStatus::Success, "CREATE STORAGE failed: {:?}", resp.error);
            ...
            anyhow::ensure!(row.get("storage_name").and_then(|v| v.as_str()) == Some("Archive Storage Updated"));
            ...
            let resp = server
                .execute_sql(&format!("DROP TABLE {}.t", ns))
                .await?;
            anyhow::ensure!(resp.status == ResponseStatus::Success);
