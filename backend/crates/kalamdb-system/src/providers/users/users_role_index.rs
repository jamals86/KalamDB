...
impl RoleIndex {
    ...
    /// # Returns
    /// Result indicating success or failure
    pub fn remove_user(&self, user_id: &UserId, role: &Role) -> Result<(), SystemError> {
    ...
    pub fn lookup(&self, role: &Role) -> Result<Vec<UserId>, SystemError> {
        let role_key = role_to_key(role);
    ...
    fn get_entry(&self, key: &[u8]) -> Result<Option<RoleIndexEntry>, SystemError> {
    ...
    fn put_entry(&self, key: &[u8], entry: &RoleIndexEntry) -> Result<(), SystemError> {
